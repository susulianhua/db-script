...
public class FileObjectImpl extends AbstractFileObject {
    ...
    public FileObjectImpl( String resource) {
    ...
    public List<FileObject> getChildren() {
        if (isModified() || CollectionUtils.isEmpty(children)) {
            ...
            if (subFiles != null && subFiles.length > 0) {
                for (File subfile : subFiles) {
                    ...
                    if (subfile.getName().endsWith(".jar")
                            && !subfile.isDirectory()) {
                        fileObject = new FileObjectImpl(subfile.getAbsolutePath());
                    } else {
                        fileObject = new FileObjectImpl(subfile);
