...
public abstract class NodeImpl<T extends Node<T>, NT extends NodeType>
    ...
    public List<T> getSubNodesByPath(String nodePath) {
        // 路径为空或者子节点列表为空返回null
        if (StringUtils.isBlank(nodePath) || subNodes == null)
            return null;
        nodePath = getCaseSensitiveName(nodePath);
     ...
     * @return
     */
    private List<T> checkSub(List<T> sub, String nodeName) {
        List<T> res = new ArrayList<T>();
        for (T t : sub) {
            if (nodeName.equals(t.getNodeName()) && t.getSubNodes() != null)
                res.addAll(t.getSubNodes());
        }
        return res;
    }
        ...
        if (attributeList != null) {
            for (Attribute attribute : attributeList) {
                ...
                sb.append(attribute.getName()).append("=").append("\"")
                        .append(encode(attribute.getValue())).append("\"");
            }
        }
