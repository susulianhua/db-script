/**
 * Copyright (c) 2012-2017, www.tinygroup.org (luo_guo@icloud.com).
 * <p>
 * Licensed under the GPL, Version 3.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * <p>
 * http://www.gnu.org/licenses/gpl.html
 * <p>
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package org.tinygroup.earthworm.test;

import org.tinygroup.earthworm.EarthWorm;
import org.tinygroup.earthworm.impl.CloseSampleTrier;
import org.tinygroup.logger.LogLevel;
import org.tinygroup.logger.Logger;
import org.tinygroup.logger.LoggerFactory;

public class LocalOnlyClose {
    private static Logger LOGGER = LoggerFactory.getLogger(ForTest.class);

    public static void main(String[] args) {
        EarthWorm.setSampleTrier(CloseSampleTrier.class.getName());

        EarthWorm.startTrace("", "");
        LOGGER.logMessage(LogLevel.INFO, EarthWorm.getRpcContext() + "====================");
        EarthWorm.startLocal("a1");

        EarthWorm.startLocal("a1");
        EarthWorm.endLocal();

        EarthWorm.startLocal("a1");
        EarthWorm.endLocal();

        EarthWorm.startLocal("a1");
        EarthWorm.endLocal();

        EarthWorm.endLocal();
        LOGGER.logMessage(LogLevel.INFO, EarthWorm.getRpcContext() + "====================");
        EarthWorm.endTrace();
        LOGGER.logMessage(LogLevel.INFO, EarthWorm.getRpcContext() + "====================");
    }
}
