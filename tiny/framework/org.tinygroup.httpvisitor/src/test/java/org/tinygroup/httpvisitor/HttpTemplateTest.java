...
public class HttpTemplateTest extends TestCase {
    ...
    public void testRequestInterceptor() {
        HttpConfigTemplate template = manager.getHttpConfigTemplate("interceptor1");
        assertNotNull(template);

        List<RequestInterceptor> list = template.getInterceptorList();
        assertEquals(2, list.size());
        assertEquals(GetRequestInterceptor.class, list.get(0).getClass());
        assertEquals(PostRequestInterceptor.class, list.get(1).getClass());
    }
