...
public class ConfigManager2Test extends TestCase {
	...
	public void testRemoveMap() {
		...
		class abc {
			...
			public void deal(ConfigChangeEvent event) {
				if (event.getType().equals(ConfigChangeEvent.TYPE_DELETE)) {
					info1.appendStr(event.getNewValue());
					info1.appendKey(event.getKey());
				}
		...
		manager.removeMapItem("remote");
		try {
			Thread.sleep(2000);//因为是异步监听
		} catch (Exception e) {
		}
		
		assertEquals("1", manager.getConfig("a"));
		assertEquals(3, info1.getKey().length());
		assertEquals(2, info2.getKey().length());
	...
	public void testAddConfig() {
		...
		assertEquals(7, info.getKey().length());
