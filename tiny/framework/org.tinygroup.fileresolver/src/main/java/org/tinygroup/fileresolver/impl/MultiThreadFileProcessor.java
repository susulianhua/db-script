...
public class MultiThreadFileProcessor {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(MultiThreadFileProcessor.class);

    private MultiThreadFileProcessor() {

    }

    public static void multiProcessor(int threadNum, String mutiThreadName,
                                      List<FileObject> fileObjects, ProcessorCallBack callBack) {

        MultiThreadProcessor processors = new MultiThreadProcessor(
                mutiThreadName);
        int fileNums = fileObjects.size();
        if (fileNums <= threadNum) {
            threadNum = 1;
        }
        int numOfEachThread = fileNums % threadNum == 0 ? fileNums / threadNum
                : fileNums / threadNum + 1;
        int fromIndex = 0;
        for (int i = 0; i < threadNum; i++) {
            ...
            FileProcessorThread processor = new FileProcessorThread(
                    String.format("file-processor-thread-%d", i), files);
            processor.setCallBack(callBack);
            processors.addProcessor(processor);
            if (fromIndex > fileNums) {
                break;
            }
        }
        ...
        LOGGER.logMessage(LogLevel.INFO, "线程组:<{}>执行时间：{}", mutiThreadName,
                endTime - startTime);
