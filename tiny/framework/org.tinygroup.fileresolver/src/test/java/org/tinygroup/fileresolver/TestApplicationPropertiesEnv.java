...
public class TestApplicationPropertiesEnv {

    private final static Logger LOGGER = LoggerFactory.getLogger(TestApplicationPropertiesEnv.class);
    private static String DEFAULT_CONFIG = "applicationEnv.xml";
    ...
    public static void main(String[] args) {
        ...
        InputStream inputStream = TestApplicationPropertiesEnv.class.getClassLoader()
                .getResourceAsStream(DEFAULT_CONFIG);
        if (inputStream == null) {
            inputStream = TestApplicationPropertiesEnv.class.getResourceAsStream(DEFAULT_CONFIG);
        }
        String applicationConfig = "";
        try {
            applicationConfig = StreamUtil.readText(inputStream, "UTF-8", false);
        } catch (IOException e) {
            e.printStackTrace();
        }
        if (applicationConfig != null) {
            ConfigurationManager c = ConfigurationUtil.getConfigurationManager();
            c.setApplicationConfiguration(new XmlStringParser().parse(applicationConfig).getRoot());
        ...
        LocalPropertiesFileProcessor localProcessor = new LocalPropertiesFileProcessor(applicationConfig);
        ...
        Assert.assertEquals("127.0.0.1", map.get("schost"));
        Assert.assertEquals("9191", map.get("scport"));
        XmlNode node = ConfigurationUtil.getConfigurationManager().getApplicationConfiguration();
        LOGGER.logMessage(LogLevel.INFO, "{}", node);
        List<XmlNode> nodes1 = node.getSubNodes("value3");
        Assert.assertEquals("127.0.0.1", nodes1.get(0).getAttribute("value"));
        List<XmlNode> nodes2 = node.getSubNodes("value4");
        Assert.assertEquals("9191", nodes2.get(0).getAttribute("value"));
