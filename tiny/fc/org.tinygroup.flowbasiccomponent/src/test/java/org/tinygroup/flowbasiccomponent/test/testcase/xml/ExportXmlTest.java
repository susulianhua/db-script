 ...
 * @author qiucn
 */
public class ExportXmlTest extends AbstractFlowComponent {

    public void testExportXml() {
        Context context = ContextFactory.getContext();
        context.put("xml", getXml());
        context.put("exportFilePath",
                VFS.resolveFile("src/test/resources/export.xml")
                        .getAbsolutePath());
        context.put("nodePath", "/CFX/HEAD/MsgID");
        context.put("encoding", "GB2312");
        Event e = Event.createEvent("exportNodeFlow", context);
        cepcore.process(e);
        assertEquals(
                "<MsgID>20050830200100087356</MsgID>",
                XMLOperatorUtil.getXmlNodeFromFile(
                        VFS.resolveFile("src/test/resources/export.xml")
                                .getAbsolutePath(), "GB2312").toString());

        deleteFile(VFS.resolveFile("src/test/resources/export.xml")
                .getAbsolutePath());
    }

    public void testExportXml1() {
        Context context = ContextFactory.getContext();
        context.put("xml", getXml());
        context.put("exportFilePath",
                VFS.resolveFile("src/test/resources/export1.xml")
                        .getAbsolutePath());
