 ...
 * @author qiucn
 */
public class AssertComponentTest extends AbstractFlowComponent {

    public void testAssertComponentFalse() {
        Context context = ContextFactory.getContext();
        context.put("el", "0==1");
        context.put("resultKey", "result");
        context.put("result", "");
        Event e = Event.createEvent("assertComponentTestFlow", context);
        cepcore.process(e);
        assertEquals(false, e.getServiceRequest().getContext().get("result"));
    }

    public void testAssertComponentTrue() {
        Context context = ContextFactory.getContext();
        context.put("el", "1==1");
        context.put("resultKey", "result");
        context.put("result", "");
