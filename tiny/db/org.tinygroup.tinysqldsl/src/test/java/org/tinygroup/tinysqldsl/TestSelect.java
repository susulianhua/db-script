...
public class TestSelect {
	public static void main(String[] args) {
		...
		System.out
				.println(select(
						customSelectItem("%s-%s as remainder", CUSTOM.NAME,
								CUSTOM.AGE)).from(CUSTOM));
		...
		System.out.println(select(CUSTOM.AGE.count().as("num", true)).from(
				CUSTOM));
		...
		System.out.println(select(CUSTOM.NAME).from(
				subSelect(select(alias.ALL).from(alias), "custom", true))
				.where(CUSTOM.ID.eq("2324")));
		...
		System.out
				.println(selectFrom(CUSTOM).where(CUSTOM.AGE.notIn(1, 5, 10)));

		System.out.println(selectFrom(CUSTOM).where(
				CUSTOM.AGE.notIn(null, null, null)));

		System.out.println(selectFrom(CUSTOM).where(
				CUSTOM.AGE.notIn(1, null, 10)));

		System.out.println(selectFrom(CUSTOM).where(
				ExistsExpression.existsCondition(subSelect(selectFrom(CUSTOM)
						.where(CUSTOM.AGE.in(10000, 100001))))));

		System.out.println(selectFrom(CUSTOM).where(
				ExistsExpression
						.notExistsCondition(subSelect(selectFrom(CUSTOM).where(
								CUSTOM.AGE.in(10000, 100001))))));

		System.out.println(selectFrom(CUSTOM).where(
				CUSTOM.AGE.inExpression(subSelect(select(CUSTOM.ID)
						.from(CUSTOM).where(CUSTOM.AGE.in(10000, 100001))))));
		System.out.println(select(CUSTOM.ALL).from(CUSTOM, TSCORE));

		System.out.println(select(CUSTOM.NAME).from(CUSTOM, TSCORE).where(
				and(CUSTOM.AGE.greaterThan(null), CUSTOM.NAME.equal(null),
						CUSTOM.ID.eq(TSCORE.ID))));
		...
		System.out.println(select(manName, womonName).from(man, womon).where(
				manName.eq("test")));

		System.out.println(select(CUSTOM.ALL, TSCORE.ALL).from(CUSTOM)
				.join(simpleJoin(TSCORE)).where(CUSTOM.ID.eq(1)));

		System.out.println(select(CUSTOM.AGE.distinct().count().as("sffs"))
				.from(CUSTOM));

		System.out.println(select(CUSTOM.AGE.distinct().as("sffs"))
				.from(CUSTOM));

		CUSTOM.ID.setAlias(new Alias("dsds"));

		System.out.println(select(CUSTOM.ID).from(CUSTOM)
				.where(CUSTOM.ID.equal(111))
				.orderBy(asc(CUSTOM.ID), desc(CUSTOM.NAME)));

		Select select = Select
				.selectFrom(CUSTOM)
				.where(or(and(CUSTOM.NAME.eq(null), CUSTOM.ID.eq("1222")),
						and(CUSTOM.NAME.eq(null), CUSTOM.AGE.eq("1222"))))
				.orderBy(OrderByElement.asc(CUSTOM.NAME));
		System.out.println(select.sql());

		select = Select
				.selectFrom(CUSTOM)
				.where(CUSTOM.AGE.eq("1222").and(
						or(CUSTOM.NAME.eq("dsfs"), CUSTOM.NAME.eq("ssdd"))))
				.orderBy(OrderByElement.asc(CUSTOM.NAME));
		System.out.println(select.sql());

		CustomTable CUSTOM_ALIASA = CUSTOM.as("a");
		CustomTable CUSTOM_ALIASB = CUSTOM.as("b");
		System.out.println(Select.selectFrom(CUSTOM_ALIASA).where(
				CUSTOM_ALIASA.NAME.notInExpression(SubSelect.subSelect(select(
						CUSTOM_ALIASB.NAME).from(CUSTOM_ALIASB).where(
						CUSTOM_ALIASB.ID.leftLike("admin"))))));
