...
public abstract class StatementSqlBuilder {
    ...
    public StringBuilder getLimitString(String sql, int start, int limit) {
        ...
        if (start == 0) {
            pagingSelect.append(sql).append(" fetch first ").append(limit).append(" rows only");
            return pagingSelect;
        }
        pagingSelect.append("select * from ( select  inner2_.*, rownumber() over(order by order of inner2_) as rownumber_ from (") //add another (inner) nested select
                .append(sql) //add the main query
                .append(" fetch first ")
                .append(start + limit)
                .append(" rows only ) as inner2_ ) as inner1_ where rownumber_ > ")
                .append(start).append(" order by rownumber_");
        return pagingSelect;
    }

    /**
     * 原有limit语法，db2下没表名会报错
     * @param sql
     * @param start
     * @param limit
     * @return
     */
    @Deprecated
    private StringBuilder getOldLimitString(String sql, int start, int limit) {
