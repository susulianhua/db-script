...
public class TG_WSServletContextListener implements
    ...
    public void contextInitialized(ServletContextEvent event) {
        ...
        String targetNamespace = context
                .getInitParameter(TinyListenerProcessor.TARGET_NAMESPACE);
        if (StringUtil.isBlank(targetNamespace)) {
            if (StringUtil.isBlank(contextPath)) {
                targetNamespace = WebserviceUtil.PACKAGE_NAME + "."
                        + DEFAULT_CONTEXT_PATH_NAME;
            } else {
                targetNamespace = WebserviceUtil.PACKAGE_NAME + "."
        ...
        for (ServiceInfo serviceInfo : serviceInfos) {
            try {
                ...
                createService(targetNamespace, serviceInfo, classLoader,
                        context);
                logger.logMessage(LogLevel.INFO,
    ...
    private void createService(String packageName, ServiceInfo serviceInfo,
        ...
        if (sunJaxWsXml == null)
            throw new WebServiceException(
                    WsservletMessages.NO_SUNJAXWS_XML(JAXWS_RI_RUNTIME + " and " + JAXWS_RI_RUNTIME2));
     ...
     */
    protected @NotNull
    WSServletDelegate createDelegate(
