 ...
 */
public class SessionWebContextImpl extends AbstractWebContextWrapper implements
        SessionWebContext {
    private final static Logger log = LoggerFactory
            .getLogger(SessionWebContextImpl.class);
    ...
    private boolean ignoreSessionTouchCheck;
    ...
    private void ensureRequestedSessionID() {
        if (!requestedSessionIDParsed) {
            ...
            if (requestedSessionID == null
                    && sessionConfig.getId().isUrlEncodeEnabled()) {
    ...
    private void writeSessionIDCookie(String cookieValue) {
        ...
        CookieSupport cookie = new CookieSupport(cookieConfig.getName(),
                cookieValue);
    ...
    public String decodeSessionIDFromCookie() {
        ...
        if (cookies != null) {
            String sessionCookieName = sessionConfig.getId().getCookie()
                    .getName();
    ...
    public String encodeSessionIDIntoURL(String url) {
        ...
        if (session != null
                && (session.isNew() || isRequestedSessionIDFromURL()
                && !isRequestedSessionIDFromCookie())) {
            String sessionID = session.getId();
            String keyName = getSessionConfig().getId().getUrlEncode()
                    .getName();
            ...
            for (int keyBeginIndex = url.indexOf(';'); keyBeginIndex >= 0
                    && keyBeginIndex < urlLength; keyBeginIndex = url.indexOf(
                    ';', keyBeginIndex + 1)) {
                keyBeginIndex++;

                if (urlLength - keyBeginIndex <= keyNameLength
                        || !url.regionMatches(keyBeginIndex, keyName, 0,
                        keyNameLength)
                ...
                if (!url.regionMatches(valueBeginIndex, sessionID, 0,
                        sessionID.length())) {
            ...
            if (!found) {
                url = url.substring(0, urlLength) + ';' + keyName + '='
                        + sessionID + url.substring(urlLength);
            }
        }
    ...
    public HttpSession getSession(boolean create) {
        ...
        if (session == null) {
            ...
            if (sessionID == null) {
                ...
                sessionID = sessionConfig.getId().getGenerator()
                        .generateSessionID();
                isNew = true;
            }
        ...
        if (sessionConfig.getId().isCookieEnabled()
                && !session.getId().equals(requestedSessionID)) {
	...
	public boolean isIgnoreSessionTouch() {
		return ignoreSessionTouchCheck;
	}
	

	public void setIgnoreSessionTouch(boolean ignore){
    	this.ignoreSessionTouchCheck=ignore;
    }
}
