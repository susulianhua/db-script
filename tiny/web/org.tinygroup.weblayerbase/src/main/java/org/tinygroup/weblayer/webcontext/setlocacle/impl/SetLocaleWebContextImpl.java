...
/**
 * <code>SetLocaleWebContext</code>的实现。
 *
 * @author renhui
 */
public class SetLocaleWebContextImpl extends AbstractWebContextWrapper
        implements SetLocaleWebContext {

    protected static Logger log = LoggerFactory
            .getLogger(SetLocaleWebContextImpl.class);
    ...
    public void setResponseContentType(String contentType, boolean appendCharset) {
        ((ResponseWrapper) getResponse()).setContentType(contentType,
                appendCharset);
    }
    ...
    public void prepare() {
        ...
        if (PARAMETER_SET_TO_DEFAULT_VALUE.equalsIgnoreCase(getRequest()
            ...
            if (paramLocale != null) {
                getRequest().getSession().setAttribute(sessionKey,
                        paramLocale.toString());
            ...
            if (outputCharset != null) {
                localeInfo = new LocaleInfo(localeInfo.getLocale(),
                        outputCharset);
            }
        }
        ...
        log.logMessage(LogLevel.DEBUG, "Set OUTPUT locale:charset to {}",
                localeInfo);

        // 设置thread context中的locale信息。
        LocaleUtil.setContext(localeInfo.getLocale(), localeInfo.getCharset()
                .name());
        log.logMessage(LogLevel.DEBUG, "Set THREAD CONTEXT locale:charset to {}",
                localeInfo);
    ...
    private LocaleInfo getLocaleInfoFromSessionOrGetTheDefaultValue() {
        ...
        String localeName = session == null ? null : (String) session
        ...
        if (isEmpty(localeName)) {
            ...
            if (!LocaleUtil.isLocaleSupported(localeInfo.getLocale())
                    || !LocaleUtil.isCharsetSupported(localeInfo.getCharset()
                    .name())) {
                log.logMessage(LogLevel.WARN, "Invalid locale {} from session",
                        localeInfo);
    ...
    /**
     * 设置input charset。 假如query string中指定了input charset，则采用之。 否则使用参数所指定的值作为input
     * charset。
     *
     * @param inputCharset 默认input charset，或是从session中所取得的input charset
     * @param overrider    如果有，则从中取得input charset
     */
    private void setInputCharsetToRequest(String inputCharset,
                                          SetLocaleOverrider overrider) {
        try {
            ...
            try {
                ...
                log.logMessage(LogLevel.WARN,
                        "Unknown charset {}.  Set INPUT charset to {}",
                        inputCharset, CHARSET_DEFAULT);
            } catch (UnsupportedEncodingException ee) {
                log.errorMessage("Failed to set INPUT charset to {}", ee,
                        CHARSET_DEFAULT);
            }
        }
    }
        ...
        if (queryString != null) {
            ...
            if (matcher.find()) {
                ...
                if (LocaleUtil.isCharsetSupported(charset)) {
                    ...
                    log.logMessage(LogLevel.WARN,
                            "Specified input charset is not supported: {}",
                            charset);
                }
            }
        }
        ...
        if (overrider != null) {
            ...
            if (charset != null) {
                if (LocaleUtil.isCharsetSupported(charset)) {
                    ...
                    log.logMessage(
                            LogLevel.WARN,
                            "Specified overridden input charset is not supported: {}",
                            charset);
                }
            }
        }
    ...
    private LocaleInfo resetLocaleInfoInSession() {
        ...
        log.logMessage(LogLevel.DEBUG, "Reset OUTPUT locale:charset to {}",
                localeInfo);
        ...
        if (queryString != null) {
            ...
            if (matcher.find()) {
                ...
                if (LocaleUtil.isCharsetSupported(charset)) {
                    ...
                    log.logMessage(LogLevel.WARN,
                            "Specified output charset is not supported: {}",
                            charset);
                }
            }
        }
        ...
        if (overrider != null) {
            ...
            if (charset != null) {
                if (LocaleUtil.isCharsetSupported(charset)) {
                    ...
                    log.logMessage(
                            LogLevel.WARN,
                            "Specified overridden output charset is not supported: {}",
                            charset);
                }
            }
        }
    ...
    private LocaleInfo getLocaleInfoFromParameter() {
        ...
        if (!isEmpty(localeName)) {
            ...
            if (!LocaleUtil.isLocaleSupported(localeInfo.getLocale())
                    ...
                    .name())) {
                log.logMessage(LogLevel.WARN,
                        "Invalid locale {} from request parameters", localeInfo);
    ...
    private class ResponseWrapper extends AbstractResponseWrapper {
        ...
        public void setContentType(String contentType, boolean appendCharset) {
            ...
            String charset = trimToNull(substringAfterLast(contentType,
                    "charset="));
        ...
        public void setCharacterEncoding(String charset) {
            ...
            if (contentType != null) {
                ...
                log.logMessage(LogLevel.DEBUG, "Set content type to {}",
                        contentType);
