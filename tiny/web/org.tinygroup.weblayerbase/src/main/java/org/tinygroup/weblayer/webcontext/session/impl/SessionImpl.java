...
public class SessionImpl implements HttpSession, Serializable, Session {
			...
			if (model == null) {
				...
				if (expired) {
					...
					log.logMessage(
							...
							modelSessionID, new Date(model.getCreationTime()),
							new Date(model.getLastAccessedTime()), model
					...
					if (!webContext.isIgnoreSessionTouch()) {
						model.touch();
					}
				}
			}
	...
	public void invalidate() {
		...
		if (!StringUtil.isBlank(webContext.getSessionConfig()
				.getSessionManagerBeanId())) {
			SessionManager manager = SessionManagerFactory.getSessionManager(
					webContext.getSessionConfig().getSessionManagerBeanId(),
		...
		if (mayNotBeCommitted != null) {
			for (Map.Entry<String, StringBuilder> entry : mayNotBeCommitted
				...
				log.logMessage(
						...
						store.getClass().getSimpleName(), storeName, attrNames);
			}
		}
