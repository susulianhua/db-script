...
public class ParserTinyFilter extends AbstractTinyFilter {
    ...
    private static final String EMPTY_TO_NULL = "emptyToNull";
    ...
    private void init() {
        ...
        if (emptyToNull == null) {
            // 是否对参数值进行空字符串转null处理
            emptyToNull = ObjectUtil.defaultIfNull(
                    Boolean.parseBoolean(get(EMPTY_TO_NULL)), false);
        }
    ...
    public WebContext getAlreadyWrappedContext(WebContext wrappedContext) {
        ...
        parserWebContext.setEmptyToNull(emptyToNull);
