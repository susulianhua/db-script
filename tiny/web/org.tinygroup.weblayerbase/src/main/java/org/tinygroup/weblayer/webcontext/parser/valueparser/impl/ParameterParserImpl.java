...
public class ParameterParserImpl extends AbstractValueParser implements
    ...
    private void fileItemFilter(String key, boolean[] filtering, Object value,
        ...
        if (processFileItem == null) {
            ...
            if (!fileItem.isFormField() && fileItem.getContentType() != null) {// 文件类型
    ...
    public void parseUpload(UploadParameters params) throws UploadException {
        ...
        for (FileItem item : items) {
            ...
            if (item instanceof InMemoryFormFieldItem) {
                ...
                if (tinyFileItem.isInMemory()) {
                    fileObject = new TinyItemFileObject(tinyFileItem);
                } else {
                    fileObject = tinyFileItem.getFileObject();
                }
            }
