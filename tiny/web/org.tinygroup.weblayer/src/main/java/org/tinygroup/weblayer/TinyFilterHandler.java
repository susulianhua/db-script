 ...
 * @author renhui
 */
public class TinyFilterHandler implements FilterHandler {
    ...
    public void tinyFilterProcessor(HttpServletRequest request,
        ...
        try {
            ...
            // 可能之前被修改过
            if (isRequestPassedThru(servletPath)
     ...
     * 该功能适用于仅将weblayer视作普通的filter，而filter
     * chain的接下来的部分将可使用weblayer所提供的TinyFilter功能。
     *
     * @param servletPath
     * @return
     */
            ...
            for (Pattern pattern : passthruPatterns) {
                if (pattern.matcher(servletPath).matches()) {
                    logger.logMessage(LogLevel.DEBUG,
                            "Passed through request: {0}", servletPath);
                    return true;
                }
            }
        }
