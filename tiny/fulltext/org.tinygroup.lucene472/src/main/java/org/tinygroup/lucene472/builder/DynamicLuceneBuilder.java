...
public class DynamicLuceneBuilder extends BaseLuceneBuilder {
    ...
    public DynamicLuceneBuilder(String userId, String configId) {
        super();
        this.userId = userId;
    ...
    public LuceneDynamicManager getLuceneDynamicManager() {
        if (luceneDynamicManager == null) {
            luceneDynamicManager = beanContainer.getBean(LuceneDynamicManager.DEFAULT_BEAN_NAME);
        }
    ...
    public LuceneConfig buildConfig() {
        return getLuceneDynamicManager().getFullTextConfig(userId, configId);
    }
